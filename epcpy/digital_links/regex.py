...
DIGIT = "\d"
BOOLEAN = "(0|1)"
UPPERALPHA = "(%(4[1-9A-F]|5[0-9A])|[A-Z])"
LOWERALPHA = "(%(6[1-9A-F]|7[0-9A])|[a-z])"
...
SHIP_TO_VALUE = f"{DIGIT}{{13}}"
BILL_TO_VALUE = f"{DIGIT}{{13}}"
PURCHASED_FROM_VALUE = f"{DIGIT}{{13}}"
GLN_VALUE = f"{DIGIT}{{13}}"
SHIP_FOR_VALUE = f"{DIGIT}{{13}}"
PAY_TO_VALUE = f"{DIGIT}{{13}}"
GLN_PROD_VALUE = f"{DIGIT}{{13}}"
...
SHIP_TO_COMP = f"\/{SHIP_TO_CODE}\/{SHIP_TO_VALUE}"
BILL_TO_COMP = f"\/{BILL_TO_CODE}\/{BILL_TO_VALUE}"
PURCHASED_FROM_COMP = f"\/{PURCHASED_FROM_CODE}\/{PURCHASED_FROM_VALUE}"
GLN_COMP = f"\/{GLN_CODE}\/{GLN_VALUE}"
SHIP_FOR_COMP = f"\/{SHIP_FOR_CODE}\/{SHIP_FOR_VALUE}"
PAY_TO_COMP = f"\/{PAY_TO_CODE}\/{PAY_TO_VALUE}"
GLN_PROD_COMP = f"\/{GLN_PROD_CODE}\/{GLN_PROD_VALUE}"
...
SHIP_TO_PATH = f"{SHIP_TO_COMP}"
BILL_TO_PATH = f"{BILL_TO_COMP}"
PURCHASED_FROM_PATH = f"{PURCHASED_FROM_COMP}"
GLN_PATH = f"{GLN_COMP}({GLNX_COMP})?"
SHIP_FOR_PATH = f"{SHIP_FOR_COMP}"
PAY_TO_PATH = f"{PAY_TO_COMP}"
GLN_PROD_PATH = f"{GLN_PROD_COMP}"
...
URI_SAFE_BASE_64_CHAR = f"({DIGIT}|{ALPHA}|_|-)"
COMPRESSED_SEGMENT = f"{URI_SAFE_BASE_64_CHAR}"

PRIMARY_ID_COMPONENT = (
    f"({GTIN_COMP}|{ITIP_COMP}|{GMN_COMP}|{CPID_COMP}"
    f"|{SHIP_TO_COMP}|{BILL_TO_COMP}|{PURCHASED_FROM_COMP}"
    f"|{SHIP_FOR_COMP}|{GLN_COMP}|{PAY_TO_COMP}|{GLN_PROD_COMP}"
    f"|{GSRNP_COMP}|{GSRN_COMP}|{GCN_COMP}|{SSCC_COMP}"
    f"|{GDTI_COMP}|{GINC_COMP}|{GSIN_COMP}|{GRAI_COMP}|{GIAI_COMP})"
)
PARTIALLY_COMPRESSED_GS1_WEB_URI_PATH = (
    f"({PRIMARY_ID_COMPONENT}\/{COMPRESSED_SEGMENT})"
)
PARTIALLY_COMPRESSED_GS1_WEB_URI_PATTERN = (
    f"({PARTIALLY_COMPRESSED_GS1_WEB_URI_PATH}({QUERY_STRING_COMP})?)"
)
PARTIALLY_COMPRESSED_REFERENCE_GS1_WEB_URI = (
    f"(https://id.gs1.org{PARTIALLY_COMPRESSED_GS1_WEB_URI_PATTERN})"
)
PARIALLY_COMPRESSED_CUSTOM_GS1_WEB_URI = (
    f"({CUSTOM_URI_STEM}{PARTIALLY_COMPRESSED_GS1_WEB_URI_PATTERN})"
)

FULLY_COMPRESSED_GS1_WEB_URI_PATTERN = f"({COMPRESSED_SEGMENT}({QUERY_STRING_COMP})?)"
FULLY_COMPRESSED_REFERENCE_GS1_WEB_URI = (
    f"(https://id.gs1.org{FULLY_COMPRESSED_GS1_WEB_URI_PATTERN})"
)
FULLY_COMPRESSED_CUSTOM_GS1_WEB_URI = (
    f"({CUSTOM_URI_STEM}{FULLY_COMPRESSED_GS1_WEB_URI_PATTERN})"
)
